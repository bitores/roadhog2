...
function createProxy(method, path, target, opt) {
  opt = opt || {};
  return proxy(target, Object.assign({
    filter(req) {
...
function realApplyMock(devServer) {
  ...
  Object.keys(config).forEach(key => {
    ...
    if (Object.prototype.toString.call(config[key]) === '[object String]') {
      ...
      app.use(path, createProxy(keyParsed.method, path, config[key], null));
    } else if (Object.prototype.toString.call(config[key]) === '[object Array]') {
      let { path } = keyParsed;
      if (/\(.+\)/.test(path)) {
        path = new RegExp(`^${path}$`);
      }
      app.use(path, createProxy(keyParsed.method, path, config[key][0], config[key][2]));
    } else {
